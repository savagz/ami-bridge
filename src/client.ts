...
export default class Client extends EventEmitter {
  ...
  connect(reconnectable?: boolean, reconnectTimeout?: number) {
    if (reconnectable) {
  ...
  initSocket() {
    ...
    this.socket.on('close', (hadError: any) => {
  ...
  auth(message: any) {
    ...
    if (String(message).match(/Asterisk Call Manager/)) {
      ...
      this.send(new Actions.Login(this.config.login, this.config.password), (error: any, response: any) => {
        if (error) {
  ...
  parseMessage(raw: string) {
    ...
    if (raw.match(/^Response: /)) {
      ...
      if ((response as any).actionid) this.responses[(response as any).actionid] = response;
  ...
  _parseUnformatMessage(raw: string) {
    ...
    (Response as any).tryFormat({ raw }, (err: any, data: any) => {
      if (err) return this.logger.warn('Fail fromat:', err);
      if (!this.unformatWait[data.type]) return this.logger.warn("Doesn't wait:", data.type);
      if (!this.unformatWait[data.type].res || this.unformatWait[data.type].res.length === 0) {
        if (data.res.length === 1) this.unformatWait[data.type].res = data.res;
        else {
      ...
      } else this.unformatWait[data.type].res.push(data.res[0].replace('%REPLACE_ACTION_ID%', this.unformatWait[data.type].id));
      clearTimeout(this.unformatWait.queues.timeout);
      if (this.unformatWait[data.type].res.length === 1) return this.emit('needParseMessage', this.unformatWait[data.type].res[0]);
      (this.unformatWait[data.type].timeout = setTimeout(() => {
        ...
        this.unformatWait[data.type].res.forEach((mes: any) => {
  ...
  reconnect(reconnectable?: boolean) {
  ...
  useLogger(loggerToUse: any) {
    this.logger = loggerToUse;
  }

  send(action: any, callback: any) {
    ...
    this.socket!.write(action.format(), (err: any) => {
      if (err) {
        return callback(err);
      }
      let timeout: any;
      (timeout = setTimeout(() => {
        if (this.callbacks[action.ActionID]) this.callbacks[action.ActionID](new Error('ERRTIMEDOUT'));
      }, 300000)).unref();
      this.callbacks[action.ActionID] = (err2: any, data: any) => {
