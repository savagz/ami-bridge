function ensureIfNoDefaultsForOptional(defaults: Record<string, any>, optional: string[]) {
...
function ensureIfOptionalExistsInParams(params: string[], optional: string[]) {
  optional.forEach((fieldName) => {
...
function getArgsObject(args: any[], params: string[]) {
  const result = params.reduce((resultLocal: any, field, i) => {
    if (i < args.length) resultLocal[field] = args[i];
    return resultLocal;
  }, {});
  return result;
}

function applyDefaults(self: any, defaults: Record<string, any>) {
...
export default function createActionSubClass(
  {
    name,
